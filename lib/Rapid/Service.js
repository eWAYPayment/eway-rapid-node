...
function makeRequest(method, uri, data) {
  ...
  if (key && password) {
    options.headers.Authorization = 'Basic ' + Buffer.from( key + ':' + password ).toString('base64');
  }
  return fetch(parseUri(uri), options).then(function(response){ return response.json(); });
}
